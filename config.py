...
class ScriptArguments:
    ...
    # 🚀 ADD THESE LINES for pruning hyperparameters
    pruning_start_round: Optional[int] = field(default=40, metadata={"help": "Round to start pruning"})
    pruning_end_round: Optional[int] = field(default=200, metadata={"help": "Round to end pruning"})
    pruning_target: Optional[float] = field(default=0.5, metadata={"help": "Target pruning rate"})
    pruning_initial: Optional[float] = field(default=0.2, metadata={"help": "Initial pruning rate"})
...
def save_config(script_args, fed_args):
    ...
    while True:
        if not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)  # FIX: creates all intermediate dirs
            break
        else:
